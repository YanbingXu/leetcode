...
public class shoppingList {
    public static void main(String[] args) {
        ...
        while(in.hasNextLine()){
            ...
            int N = Integer.parseInt(split[0]); //总钱数
            int M = Integer.parseInt(split[1]); //希望购买物品的个数

            int[] v=new int[M];//v代表价格
            int[] p=new int[M];//p代表重要度
            int[] q=new int[M];//q代表是主件还是附件

            int result=0;

            for(int i=0;i<M;i++){
                String[] split1 = in.nextLine().split(" ");
                v[i]= Integer.parseInt(split1[0]);
                p[i]= Integer.parseInt(split1[1]);
                q[i]= Integer.parseInt(split1[2]);
            }
